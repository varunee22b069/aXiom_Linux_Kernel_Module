...
/* Enabled for touch pointer */
#define AXIOM_USE_TOUCHSCREEN_INTERFACE 1
#define AXIOM_USE_KERNEL_SLOT_ASSIGNMENT 1
#define U46_ENABLE_RAW_FORCE_DATA 0
...
u8 axiom_populate_usage_table(struct axiom_data_core *data_core, u8 *pRX_data)
{
	...
	for (usage_id = 0; usage_id < pU31_info->num_usages; usage_id++) {
		u8 offset = (usage_id * U31_BYTES_PER_USAGE);
							...
							u8 offset)
{
		...
		for (usage_index = 0; usage_index < pU31_info->num_usages; usage_index++) {
			if (pUsage_Table[usage_index].id == usage) {
				if (page < pUsage_Table[usage_index].num_pages) {
					...
					dev_err(data_core->pDev, "ERROR: aXiom-core: Invalid usage table! usage: %u, page: %u, offset: %u\n", usage, page, offset);
				}
				break;
			}
		}
	...
	dev_dbg(data_core->pDev, "aXiom-core: target_address is 0x%04x for usage: %u page %u\n", target_address, usage, page);
...
bool axiom_discover(struct axiom_data_core *data_core)
{
	...
	u16 target_address = usage_to_target_address(data_core, 0x31, 0, 0);

	if (axiom_read_page(data_core->pAxiomData, target_address, AX_U31_PAGE0_LENGTH, pRX_data) == 0) {
		dev_err(pDev, "Failed %s\n", __func__);
		return false;
	}
	axiom_get_dev_info(data_core, pRX_data);
	print_device_info(data_core);
	// Allocate memory for the usage table
	data_core->usage_table = kmalloc((sizeof(struct usage_Entry) * data_core->u31_Info.num_usages), GFP_KERNEL);
	// Read the second page of U31 to get the usage table
	// TODO: Handle if U31 has more than 1 page for usage entries; i.e. when
	//       num_usages exceed 42 (page is 256 bytes, usage entry is 6 bytes)
	target_address = usage_to_target_address(data_core, 0x31, 1, 0);
	if (axiom_read_page(data_core->pAxiomData, target_address, (U31_BYTES_PER_USAGE * data_core->u31_Info.num_usages), pRX_data) == 0) {
		dev_err(pDev, "Failed %s\n", __func__);
		return false;
	}
...
void axiom_rebaseline(struct axiom_data_core *data_core)
{
	...
	target_address = usage_to_target_address(data_core, 0x02, 0, 0);
	if (sizeof(buffer) == axiom_write_page(data_core->pAxiomData, target_address, sizeof(buffer), buffer))
...
void axiom_process_report(struct axiom_data_core *data_core, u8 *pReport)
{
	...
	case USAGE_2DCTS_REPORT_ID:
		dev_dbg(pDev, "aXiom-rep: Got touch report! (%d bytes)\n", len);
		axiom_process_u41_report(&pReport[1], data_core);
		break;

	case USAGE_2AUX_REPORT_ID: // This is an aux report (force)
		dev_dbg(pDev, "aXiom-rep: Got aux force report! (%d bytes)\n", len);
		break;

	case USAGE_2HB_REPORT_ID:
		// This is a heartbeat report
		dev_dbg(pDev, "aXiom-rep: Got Heartbeat report! (%d bytes)\n", len);
		break;

	default:
		dev_info(pDev, "aXiom-rep: got other report 0x%x (%d bytes)\n", usage, len);
		dev_info(pDev, "BUF: %*ph", len, pReport);
									 ...
									 struct u41_Target_Report *pTarget)
{
	...
	if (pTarget->index >= U41_MAX_TARGETS) {
		dev_dbg(data_core->pDev, "Invalid target index! %u\n", pTarget->index);
		return false;
	}
	...
	if (pTargetPrevState->state == currentState && pTargetPrevState->x == pTarget->x && pTargetPrevState->y == pTarget->y && pTargetPrevState->z == pTarget->z) {
	...
	slot = pTarget->index;
#endif
	dev_dbg(data_core->pDev, "axiom U41 Target T%u, slot:%u present:%u, x:%u, y:%u, z:%d\n",
...
//                   [1]: Status LSB
//                   [2]: Status MSB
//          targets: Pointer to an axiom_data_core struct
void axiom_process_u41_report(u8 *rx_buf, struct axiom_data_core *data_core)
{
	...
	input_dev = data_core->input_dev;
	if (rx_buf[0] != 0x41) {
		dev_err(data_core->pDev, "ERROR: aXiom-core: data in buffer does not have expected u41 format.\n");
		return;
	}
...
//          Returns a pointer to the register input device.
struct input_dev *axiom_register_input_subsystem(struct axiom_data_core *data_core)
{
	...
	if (data_core == NULL)
		return NULL;
	if (input_dev == NULL) {
		dev_err(data_core->pDev, "ERROR: aXiom-core: Failed to allocate memory for input device!\n");
		return NULL;
	}
	...
	if (ret != 0) {
		dev_err(data_core->pDev, "ERROR: aXiom-core: Could not register with Input Sub-system.\n");
